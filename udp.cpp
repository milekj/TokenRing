...
namespace udp {
    ...
    static int newSenderSocket;
    static int loggerSocket;
    static sockaddr_in loggerAddress;
...
int runUDP(int argc, char **argv) {
    ...
    if (hasToken) {
        ...
        string message = readMessage(receiveSocket);
        cerr << message << endl;
        ...
        message = createMessage("Let's go!", receivePort);
    ...
    while(true) {
        ...
        else if(strcmp(type, EMPTY.c_str()) == 0) { //if I got an empty token I can send own message
